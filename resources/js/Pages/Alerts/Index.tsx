        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-gray-400 mt-1">{total} alertes au total</p>
                </div>
            </div>
                    ...
                    <div className="flex items-center gap-1 bg-[#1E293B]/80 rounded-lg p-0.5 overflow-x-auto no-scrollbar">
                        {severities.map((s) => (
                            <button
                                ...
                                className={`shrink-0 px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${sevFilter === s ? 'bg-emerald-600 text-white shadow-lg' : 'text-gray-500 hover:text-gray-300'}`}
                            >{s === 'Toutes' ? 'Toutes' : s}</button>
                        ))}
                    </div>
                    <button onClick={() => navigate('sort', 'severity')} className="flex items-center gap-1.5 px-3 py-1.5 bg-[#1E293B]/80 rounded-lg text-xs text-gray-500 hover:text-gray-300 transition-all">
            ...
            <div className="flex items-center gap-2 mb-2 flex-wrap">
                {(['all', 'pending', 'resolved'] as const).map((s) => (
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${statusFilter === s ? 'bg-emerald-600/20 text-emerald-400 border border-emerald-500/30' : 'text-gray-500 hover:text-gray-300 border border-transparent'}`}
            ...
            {last_page > 1 && (
                <div className="flex items-center justify-center gap-2 overflow-x-auto no-scrollbar">
                    <button onClick={() => goPage(current_page - 1)} disabled={current_page === 1} className="p-2 rounded-lg bg-[#1E293B]/80 text-gray-500 hover:text-white disabled:opacity-30 transition-all shrink-0">
                        <ChevronLeft className="w-4 h-4" />
                    </button>
                    {Array.from({ length: last_page }, (_, i) => (
                        <button key={i} onClick={() => goPage(i + 1)} className={`w-8 h-8 rounded-lg text-xs font-bold transition-all shrink-0 ${current_page === i + 1 ? 'bg-emerald-600 text-white shadow-lg' : 'bg-[#1E293B]/80 text-gray-500 hover:text-white'}`}>
                            {i + 1}
                        </button>
                    ))}
                    <button onClick={() => goPage(current_page + 1)} disabled={current_page === last_page} className="p-2 rounded-lg bg-[#1E293B]/80 text-gray-500 hover:text-white disabled:opacity-30 transition-all shrink-0">
