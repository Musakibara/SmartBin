...
<head>
    ...
    <style>
        ...
        .status-FULL { color: #dc2626; font-weight: 700; font-size: 8px; text-transform: uppercase; }
        .status-WARNING { color: #d97706; font-weight: 700; font-size: 8px; text-transform: uppercase; }
        .status-NORMAL { color: #059669; font-weight: 700; font-size: 8px; text-transform: uppercase; }
...
<body>
    ...
    <div class="section">
        ...
        <div class="kpis">
            ...
            <div class="kpi-box">
                <div class="value" style="color: #dc2626;">{{ $fullCount }}</div>
                <div class="label">Pleines</div>
            </div>
            <div class="kpi-box">
                <div class="value" style="color: #d97706;">{{ $warningCount }}</div>
    ...
    <div class="section">
        ...
        <table>
            ...
            <tr>
                ...
                <td class="status-{{ $bin->status }}">{{ $bin->status === 'FULL' ? 'PLEINE' : ($bin->status === 'WARNING' ? 'ATTENTION' : 'NORMALE') }}</td>
                <td>{{ $bin->battery_level }}%</td>
            </tr>
            @empty
            <tr><td colspan="6" style="text-align:center;color:#94a3b8;padding:20px;">Aucune benne enregistrée.</td></tr>
            @endforelse
        </table>
    </div>
